...
- All your project code into a single main package (`Report/`)
...
```bash
<root directory>
├── Report/             # main package (should be named after your project)
...
1. Use this template to create a new GitHub repository, say `Report`
...
2. Clone the `Report` repository

3. Open a shell into your local `Report` directory and run
    ```bash
    ./rename-template.sh Report
    ```
    
    This will coherently rename the template's project name with the one chosen by you (i.e. `Report`, in this example)
...
13. Put your main (resp. test) code in `Report/` (resp. `test/`)
...
This will execute the `__main__.py` file in the `Report` package:
```bash
poetry run python -m Report
```
